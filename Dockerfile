# Dockerfile for QA Database Debug Test
#
# DOCKER CHANGES EXPLAINED:
# 
# 1. BASE IMAGE UPGRADE:
#    - Problem: Debian Buster (node:14) packages no longer available
#    - Solution: Upgraded to node:14-bullseye (Debian 11)
#    - Reason: PostgreSQL packages removed from Buster repos
# 
# 2. POSTGRESQL INSTALLATION:
#    - Problem: Default postgresql packages not available in Buster
#    - Solution: Added manual PostgreSQL repository setup
#    - Changed: From simple apt install to manual repo + specific version
# 
# 3. CONFIGURATION PATHS:
#    - Problem: PostgreSQL 11 paths no longer valid
#    - Solution: Updated paths from /etc/postgresql/11/ to /etc/postgresql/13/
#    - Reason: Version-specific configuration directories
#
# These changes were infrastructure maintenance, not related to the test code issues.

# Use a Node.js image
# CHANGED: Upgraded from Debian Buster to Bullseye due to package availability
# ORIGINAL CODE (commented out):
# FROM node:14
FROM node:14-bullseye

# Set environment variables
ENV LANG=C.UTF-8 \
    LC_ALL=C.UTF-8 \
    POSTGRES_USER=testuser \
    POSTGRES_PASSWORD=password \
    POSTGRES_DB=testdb

# Install PostgreSQL and dependencies
# CHANGED: Added manual PostgreSQL repository setup due to package availability issues
# ORIGINAL CODE (commented out):
# RUN apt-get update && \
#     apt-get install -y --no-install-recommends postgresql postgresql-contrib && \
#     rm -rf /var/lib/apt/lists/*
RUN apt-get update && \
    apt-get install -y gnupg2 wget lsb-release && \
    echo "deb http://apt.postgresql.org/pub/repos/apt bullseye-pgdg main" > /etc/apt/sources.list.d/pgdg.list && \
    wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | apt-key add - && \
    apt-get update && \
    apt-get install -y postgresql-13 postgresql-client-13 && \
    rm -rf /var/lib/apt/lists/*

# Configure PostgreSQL authentication
# CHANGED: Updated config paths from PostgreSQL 11 to 13 directories
# ORIGINAL CODE (commented out):
# RUN echo "host all all 0.0.0.0/0 md5" >> /etc/postgresql/11/main/pg_hba.conf && \
#     echo "listen_addresses='*'" >> /etc/postgresql/11/main/postgresql.conf
RUN echo "host all all 0.0.0.0/0 md5" >> /etc/postgresql/13/main/pg_hba.conf && \
    echo "listen_addresses='*'" >> /etc/postgresql/13/main/postgresql.conf

# Start PostgreSQL, create the database, user, and decode and execute the Base64 SQL commands
RUN service postgresql start && \
    su - postgres -c "psql -c 'CREATE DATABASE testdb;'" && \
    su - postgres -c "psql -c \"CREATE USER testuser WITH PASSWORD 'password';\"" && \
    su - postgres -c "psql -c 'GRANT ALL PRIVILEGES ON DATABASE testdb TO testuser;'" && \
    echo "" | \
    base64 -d | \
    su - postgres -c "psql -d testdb -f -" && \
    service postgresql stop

# Set up the working directory for the app
WORKDIR /app
COPY . .

# Install Node.js dependencies
RUN npm install

# Run the test when the container starts
CMD ["sh", "./run_test.sh"]
